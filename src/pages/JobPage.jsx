...
const JobPage = ({ deleteJob }) => {
  ...
  const onDeleteClick = (jobId) => {
    const confirm = window.confirm(
      "Are you sure about want to delete this listing?"
    );
    if (!confirm) return;
          ...
          <div class="grid grid-cols-1 md:grid-cols-70/30 w-full gap-6">
            ...
            <aside>
              ...
              <div className="bg-white p-6 rounded-lg shadow-md mt-6">
                ...
                <Link
                  to={`/edit-job/${job.id}`}
                  className="bg-blue-500 hover:bg-blue-indigo-600 text-white text-center font-bold py-2 px-4 rounded-full w-full focus:outline-none focus:shadow-outline mt-4 block"
                >
                  Edit Job
                </Link>
                <button
